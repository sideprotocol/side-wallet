...
import { convertSecondsToTime } from '@/ui/utils/formatter';
...
export default function Index() {
  ...
  const { routeConfig } = useSkipGoState();
  return (
    <>
      <Row
        itemsCenter
        ...
        {skipRoute && (
          <Text
            ...
            {convertSecondsToTime(skipRoute.estimatedRouteDurationSeconds)}
          </Text>
        )}
      </Row>
      <Drawer
        ...
        <Column gap="lg">
          <Row justifyBetween itemsCenter>
            ...
            <Row itemsCenter gap="sm">
              {(skipRoute?.chainIDs || []).map((chainId, index) => {
                const chain = skipChains!.find((item) => item.chainID === chainId)!;
                return (
                  <Fragment key={chainId}>
                    <img className={'w-[14px] h-[14px]'} src={chain.logoURI} alt="" />
                    {index !== skipRoute!.chainIDs.length - 1 && (
          ...
          <ButtonGroup
            ...
            value={+routeConfig.goFast!}
            list={[
              { key: +true, label: 'Fastest' },
              { key: +false, label: 'Cheapest' }
            ]}
            onChange={(isGoFast) => {
              dispatch(
