...
export function genPsbtOfBIP322Simple({ message, address, networkType }: { message: string; address: string; networkType: NetworkType }) {
...
export function verifyMessageOfBIP322Simple(address: string, msg: string, signature: string, networkType: NetworkType = NetworkType.MAINNET) {
  const addressType = getAddressType(address, networkType);
...
function verifySignatureOfBIP322Simple_P2TR(address: string, msg: string, sign: string, networkType: NetworkType = NetworkType.MAINNET) {
...
function verifySignatureOfBIP322Simple_P2PWPKH(address: string, msg: string, sign: string, networkType: NetworkType = NetworkType.MAINNET) {
  const network = toPsbtNetwork(networkType);
  const outputScript = bitcoin.address.toOutputScript(address, network);
